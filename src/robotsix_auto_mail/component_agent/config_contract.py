...
class ConfigContractError(Exception):
    ...
    Mirrors ``robotsix_agent_comm.protocol.Error``'s ``(code, message, **details)``
    shape so it maps cleanly onto ``Error.to(req, code=..., message=..., **details)``.
    """
...
# bind host/port, db/store paths, IMAP/SMTP host/credentials, and
# broker-connection fields.  Derived from ``_FIELD_SPECS[*].yaml_path`` so
# the allowlist references real paths.
...
def apply_config_update(
    ...
    for key, (old, new) in audit.items():
        spec = _yaml_path_to_spec[key]
        old_r = _REDACTED if spec.field_name in _SECRET_FIELDS else old
        new_r = _REDACTED if spec.field_name in _SECRET_FIELDS else new
        redacted[key] = (old_r, new_r)
...
def _coerce_value(spec: _FieldSpec, raw: object) -> object:
    """Coerce *raw* to the type expected by *spec.kind*.
