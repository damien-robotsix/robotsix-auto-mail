...
- Removed ``default_account_id`` from ``MailAccountsConfig`` — the concept of a "default account" is eliminated. The first account in configured order is now the initial board view, and account-scoped CLI commands (``probe``, ``triage``, ``triage-set``, ``config-sync``, ``config-sync-set``, ``auth login``) require an explicit ``--account`` argument. The aggregate view is only entered when explicitly requested. Server startup opens the first configured account (or starts with an in-memory database when none are configured).
