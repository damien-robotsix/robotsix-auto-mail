...
- Fix pre-existing ruff format violations in `commands_ingest.py` and `test_cli.py` (style-only). Fix pre-existing mypy errors in `commands_serve.py` (wrong attribute name `acct.id` → `acct.account_id`) and `cli/__init__.py` (type-narrowing on reused variable names in the no-command auto-start path). Add `--preview-features audit-command` to the `uv audit` CI step for compatibility with older uv versions.
