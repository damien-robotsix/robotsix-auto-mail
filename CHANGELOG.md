...
- Render sanitised HTML email body as the primary content when a
  ``text/html`` part is present, falling back to plain-text only when
  HTML is absent.  Scripts, event handlers, ``<style>`` blocks and
  remote images are stripped before rendering.
