...
def test_init_db_reset_all_triage_to_inbox(tmp_db_path: str) -> None:
    """init_db resets all triage state to INBOX (one-time migration).

    Verifies the complete lifecycle: initial data setup, migration
    execution, idempotency, and manual rollback.
    """
    import sqlite3

    # -- Step 1: create a fresh database with mixed triage data (raw
    #    sqlite3, not init_db, so no migration watermark is set yet) --

    conn = sqlite3.connect(tmp_db_path)
    conn.executescript(_SCHEMA)

    # Insert mail_records with mixed statuses and non-trivial content.
    mail_data = [
        (
            "<alpha@x.com>", "a@x.com", "Alpha", "2025-01-01",
            "needs_reply", "Plain A", "<p>A</p>",
            '{"to":["x@x"],"cc":[]}', "[]",
        ),
        (
            "<beta@x.com>", "b@x.com", "Beta", "2025-01-02",
            "to_read", "Plain B", "<p>B</p>",
            '{"to":[],"cc":[]}', "[]",
        ),
        (
            "<gamma@x.com>", "c@x.com", "Gamma", "2025-01-03",
            "done", "Plain C", "<p>C</p>",
            '{"to":["y@y"],"cc":["z@z"]}', '[{"name":"f.txt"}]',
        ),
        (
            "<delta@x.com>", "d@x.com", "Delta", "2025-01-04",
            "waiting", "Plain D", "<p>D</p>",
            '{"to":[],"cc":[]}', "[]",
        ),
    ]
    for (
    ...
    # Insert triage_decisions for a subset.
    decision_data = [
        (
            "<alpha@x.com>",
            "TO_ANSWER",
    ...
    for mid, action, source, reason, confidence, updated_at in decision_data:
        conn.execute(
            "INSERT INTO triage_decisions "
            "(message_id, action, source, reason, confidence, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (mid, action, source, reason, confidence, updated_at),
        )
    ...
    for row in conn.execute(
        ...
        pre_content[row[0]] = row[1:]

    # Capture pre-migration statuses.
    pre_statuses = dict(
        conn.execute(
    ...
    # Sanity: 3 decisions exist before migration.
    pre_decision_count = conn.execute(
        "SELECT COUNT(*) FROM triage_decisions"
    ).fetchone()[0]
    assert pre_decision_count == 3
    ...
    try:
        ...
        # All mail_records.status = 'to_read'.
        statuses = dict(
            conn2.execute(
                "SELECT message_id, status FROM mail_records"
            ).fetchall()
        ...
        for mid, st in statuses.items():
            assert st == "to_read", f"Expected to_read for {mid}, got {st!r}"
        ...
        for row in conn2.execute(
            ...
            mid = row[0]
            assert pre_content[mid] == row[1:], (
                f"Content changed for {mid}"
            )

        # Backup tables exist with correct row counts.
        backup_decision_count = conn2.execute(
            "SELECT COUNT(*) FROM triage_decisions_backup"
        ).fetchone()[0]
        assert backup_decision_count == 3

        backup_statuses = dict(
            conn2.execute(
                "SELECT message_id, status "
                "FROM mail_records_status_backup"
            ).fetchall()
        )
        expected_backup_statuses = {
            mid: pre_statuses[mid]
            for mid in pre_statuses
            if pre_statuses[mid] != "to_read"
        }
        assert backup_statuses == expected_backup_statuses

        # Verify that backup has the exact triage_decisions data.
        for mid, action, source, reason, confidence, updated_at in (
            decision_data
        ):
            backup_row = conn2.execute(
                "SELECT action, source, reason, confidence, updated_at "
                "FROM triage_decisions_backup WHERE message_id = ?",
                (mid,),
            ).fetchone()
            assert backup_row is not None, (
                f"Missing backup row for {mid}"
            )
            assert backup_row == (
                action,
                source,
        ...
        # Watermark is set (ISO timestamp).
        wm = get_watermark(conn2, _RESET_TRIAGE_WATERMARK_KEY)
    ...
    try:
        ...
        for _mid, st in statuses3.items():
            assert st == "to_read"

        # Backup tables still exist with same data (not re-created).
        assert (
            conn3.execute(
                "SELECT COUNT(*) FROM triage_decisions_backup"
            ).fetchone()[0]
        ...
        # Watermark still set.
        wm3 = get_watermark(conn3, _RESET_TRIAGE_WATERMARK_KEY)
        assert wm3 is not None
    finally:
        conn3.close()

    # -- Step 4: manual rollback --
    conn4 = sqlite3.connect(tmp_db_path)
    try:
        _rollback_reset_triage_to_inbox(conn4)

        # Decisions restored (3 rows, exact data).
        assert (
            conn4.execute(
                "SELECT COUNT(*) FROM triage_decisions"
            ).fetchone()[0]
        ...
        for mid, action, source, reason, confidence, updated_at in (
            ...
            assert row is not None, f"Decision not restored for {mid}"
            assert row == (action, source, reason, confidence, updated_at)
        ...
        for mid, expected_status in pre_statuses.items():
            assert restored_statuses[mid] == expected_status, (
                f"Status not restored for {mid}: "
                f"expected {expected_status!r}, "
                f"got {restored_statuses[mid]!r}"
            )

        # Backup tables dropped.
        remaining = conn4.execute(
            "SELECT name FROM sqlite_master "
            "WHERE type='table' AND name IN "
            "('triage_decisions_backup', 'mail_records_status_backup')"
        ).fetchall()
        assert remaining == []

        # Watermark cleared.
        assert (
            get_watermark(conn4, _RESET_TRIAGE_WATERMARK_KEY) is None
        )
    finally:
        conn4.close()
