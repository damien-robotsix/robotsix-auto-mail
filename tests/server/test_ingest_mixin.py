"""Unit tests for _IngestMixin._handle_force_fetch."""
...
class _FakeIngestHandler(_IngestMixin):
...
def test_handle_force_fetch_idempotent_returns_early() -> None:
    ...
    handler = _FakeIngestHandler(aggregate=False)
    handler._launch_background_worker.return_value = False

    with mock.patch(
        "robotsix_auto_mail.server._ingest_mixin.threading.Thread"
    ) as mock_thread:
        handler._handle_force_fetch()
...
def test_handle_force_fetch_aggregate_spawns_per_account_threads() -> None:
    ...
    handler = _FakeIngestHandler(aggregate=True, accounts=accounts)
