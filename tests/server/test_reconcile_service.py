"""Unit tests for ReconcileService.handle_reconcile."""
...
def test_handle_reconcile_idempotent_returns_early() -> None:
    ...
    with mock.patch(
        "robotsix_auto_mail.server._reconcile_service.threading.Thread"
    ) as mock_thread:
        ReconcileService(db_path=ctx.db_path).handle_reconcile(ctx)
