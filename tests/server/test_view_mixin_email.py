...
class TestServeEmailStatus:
    ...
    def test_returns_action_text(
        ...
        handler = _FakeHandler(_fake_db_path, path="/email/test%40example.com/status")
    ...
    def test_404_when_record_not_found(
        ...
        handler = _FakeHandler(_fake_db_path, path="/email/unknown%40example.com/status")
        handler._serve_email_status()
...
class TestServeEmailDetail:
    ...
    def test_embed_mode_clears_account_cookie(
        self, _fake_db_path: str, mock_build_detail_html: mock.MagicMock
    ) -> None:
    ...
    def test_focus_draft_mode_passed_through(
        ...
        handler = _FakeHandler(_fake_db_path, path="/email/test%40example.com?draft=1")
        handler._serve_email_detail()
    ...
    def test_503_on_exception(
        ...
        handler = _FakeHandler(_fake_db_path, path="/email/test%40example.com")
        handler._serve_email_detail()
        handler._send_response.assert_called_once_with(
