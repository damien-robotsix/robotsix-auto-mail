...
class TestServeBoard:
    def test_aggregate_success(
        self, fake_db_path: str, mock_build_global_board_html: mock.MagicMock
    ) -> None:
        handler = _FakeHandler(fake_db_path, _aggregate=True)
    ...
    def test_single_account_success(
        self, fake_db_path: str, mock_build_board_html: mock.MagicMock
    ) -> None:
        handler = _FakeHandler(fake_db_path, _aggregate=False)
        handler._serve_board()
        handler._send_response.assert_called_once_with(
...
class TestServeBoardContent:
    def test_aggregate_success(
        self, fake_db_path: str, mock_build_global_board_content: mock.MagicMock
    ) -> None:
        handler = _FakeHandler(fake_db_path, _aggregate=True)
        handler.accounts = mock.MagicMock()
    ...
    def test_single_account_success(
        self, fake_db_path: str, mock_build_board_content: mock.MagicMock
    ) -> None:
        handler = _FakeHandler(fake_db_path, _aggregate=False)
        handler._serve_board_content()
        handler._serve_json.assert_called_once_with(
