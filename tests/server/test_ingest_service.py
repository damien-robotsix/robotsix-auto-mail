"""Unit tests for IngestService.handle_force_fetch."""
...
class _StubContext:
    """Stub request context carrying the attributes the service reads."""
...
def _make_accounts(
    count: int,
    *,
    base_db_path: str = "/tmp/acct",  # noqa: S108
) -> MailAccountsConfig:
    accounts = [
        MailAccount(
            account_id=f"acct-{i}",
...
def test_handle_force_fetch_idempotent_returns_early() -> None:
    """When _launch_background_worker returns False, return without spawning
    threads or redirecting."""
    ctx = _StubContext(aggregate=False)
    ctx._launch_background_worker.return_value = False

    with mock.patch(
        "robotsix_auto_mail.server._ingest_service.threading.Thread"
    ) as mock_thread:
        IngestService(db_path=ctx.db_path).handle_force_fetch(ctx)
...
def test_handle_force_fetch_aggregate_spawns_per_account_threads() -> None:
    ...
    ctx = _StubContext(aggregate=True, accounts=accounts)
    ctx._launch_background_worker.return_value = True
...
def test_handle_force_fetch_non_aggregate_single_thread() -> None:
