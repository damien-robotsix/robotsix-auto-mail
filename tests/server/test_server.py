...
# ---------------------------------------------------------------------------
# GET /folders + POST /run-folder-triage tests
# ---------------------------------------------------------------------------


def test_folders_endpoint_returns_json() -> None:
    """GET /folders returns 200 with the IMAP folder names as JSON."""
    from unittest import mock
...
def test_folders_endpoint_no_mail_config_returns_503() -> None:
    """GET /folders returns 503 when IMAP is not configured."""
    import urllib.error
    ...
    try:
        try:
            ...
            assert payload["error"] == "IMAP not configured"
        else:
            raise AssertionError("Expected HTTPError for 503")
    finally:
        server.shutdown()
...
def test_run_folder_triage_sets_and_clears_watermark() -> None:
    """POST /run-folder-triage sets triage_run:state to 'running', returns
    302, and the background thread clears it back to 'idle'."""
    import time
    from unittest import mock
    ...
    try:
        ...
        server, port = _start_test_server_with_mail_config(db_path, mail_config)
        try:
            with mock.patch("robotsix_auto_mail.imap.ImapClient"):
                resp = _post_to_path(port, "/run-folder-triage", {"folder": "Archive"})
                assert resp.status == 302
                assert resp.headers.get("Location") == "/board"

                # Poll until the background thread clears the watermark.
                deadline = time.monotonic() + 10
                state = None
                while time.monotonic() < deadline:
                    conn = _init_db(db_path, skip_migrations=True)
...
def test_run_folder_triage_missing_folder_returns_400() -> None:
    """POST /run-folder-triage with no/empty folder param returns 400."""
    fd, db_path = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    try:
        mail_config = MailConfig(
        ...
        server, port = _start_test_server_with_mail_config(db_path, mail_config)
        try:
            resp = _post_to_path(port, "/run-folder-triage", {"folder": "  "})
            assert resp.status == 400
        finally:
            server.shutdown()
    finally:
...
def test_run_folder_triage_already_running() -> None:
    """POST /run-folder-triage when triage is already running is idempotent —
    it redirects to /board without changing the watermark."""
    from robotsix_auto_mail.db import get_watermark
    from robotsix_auto_mail.db import init_db as _init_db
...
def test_run_folder_triage_system_folder_returns_400() -> None:
    r"""POST /run-folder-triage on a SPECIAL-USE folder (\Sent) returns 400."""
    from unittest import mock
    ...
    try:
        ...
        try:
            ...
            with mock.patch("robotsix_auto_mail.imap.ImapClient") as mock_cls:
                mock_client = mock_cls.return_value.__enter__.return_value
                mock_client.list_folders.return_value = [sent_mbox]

                resp = _post_to_path(port, "/run-folder-triage", {"folder": "Sent"})
                assert resp.status == 400
        finally:
            server.shutdown()
    finally:
...
def test_run_folder_triage_system_folder_force_true_bypasses_guard() -> None:
    r"""POST /run-folder-triage with force=true on a \Sent folder bypasses the
    system-folder guard and proceeds (302 + watermark cycle)."""
    import time
    from unittest import mock
...
def test_board_renders_folder_picker() -> None:
    """The /board body renders the folder picker + Triage Folder form when
    triage is not running, and keeps the forbidden /run-triage strings out."""
    server, port = _start_test_server(":memory:")
    try:
        resp = urlopen(f"http://127.0.0.1:{port}/board")
        body = resp.read().decode("utf-8")
        assert 'id="folder-picker"' in body
        assert 'action="/run-folder-triage"' in body
        assert "Triage Folder" in body
        # The forbidden manual-control strings stay absent.
        assert 'id="refresh-btn"' not in body
        assert "Run triage" not in body
        assert 'action="/run-triage"' not in body
    finally:
